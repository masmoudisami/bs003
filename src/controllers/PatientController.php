...
class PatientController
{
    ...
    public function history(): void
    {
        ...
        $patient = $this->model->getById($id);
        if ($patient) {
            $slips = $this->model->getPatientSlipsWithLines($id);
    ...
    public function historyExportCsv(): void
    {
        ...
        if ($patient) {
            ...
            header('Content-Type: text/csv; charset=UTF-8');
            header('Content-Disposition: attachment; filename="historique_' . preg_replace('/[^a-zA-Z0-9]/', '_', $patient['nom']) . '_' . date('Y-m-d') . '.csv"');
                ...
                if (empty($s['lines'])) {
                    ...
                    foreach ($s['lines'] as $line) {
                        fputcsv($output, [
                            ...
                            $line['fichier_path'] ? 'Oui (' . $line['fichier_path'] . ')' : 'Non',
