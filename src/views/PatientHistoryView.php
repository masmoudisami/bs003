...
<head>
    ...
    <style>
        body { font-family: Arial, sans-serif; margin: 20px; background: #f4f4f4; font-size: 0.85em; }
        ...
        .back { display: inline-block; margin-bottom: 10px; text-decoration: none; color: #333; font-size: 0.9em; padding: 6px 12px; background: #f8f9fa; border-radius: 3px; }
        .back:hover { background: #e9ecef; }
        .btn { padding: 8px 12px; background: #17a2b8; color: #fff; text-decoration: none; border-radius: 4px; display: inline-block; margin-right: 5px; font-size: 0.85em; border: none; cursor: pointer; }
        ...
        .filters { background: #f8f9fa; padding: 15px; border-radius: 5px; margin-bottom: 15px; border: 1px solid #ddd; }
        .filters h3 { margin: 0 0 10px 0; color: #333; font-size: 1em; }
        .filters form { display: flex; flex-wrap: wrap; gap: 10px; align-items: flex-end; }
        .filters label { display: block; margin-bottom: 3px; font-weight: bold; font-size: 0.85em; }
        .filters input { padding: 6px 10px; border: 1px solid #ddd; border-radius: 3px; font-size: 0.85em; }
        .filters button { padding: 6px 15px; background: #007bff; color: #fff; border: none; border-radius: 3px; cursor: pointer; font-size: 0.85em; }
        ...
        .result-info { background: #d1ecf1; color: #0c5460; padding: 8px 12px; border-radius: 4px; margin-bottom: 15px; border-left: 4px solid #17a2b8; font-size: 0.85em; }
        .user-menu { display: flex; align-items: center; gap: 10px; white-space: nowrap; margin-left: auto; }
        .user-menu span { color: #fff; font-weight: 500; }
        .user-menu a { padding: 6px 12px; border-radius: 3px; text-decoration: none; color: #fff; font-size: 0.9em; transition: opacity 0.3s; }
        ...
        @media print { 
            ...
            .patient-info, .slip-info, .financial-summary, .comment-box, .filters { background: #fff; border: 1px solid #ddd; }
    ...
    <div class="container">
        ...
        <div class="patient-info">
            ...
            <div style="margin-left: auto; text-align: right;">
                ...
                <div class="no-print" style="margin-top: 8px;">
                    <a href="index.php?controller=patient&action=historyExportCsv&id=<?= $patient['id'] ?>&date_debut=<?= $_GET['date_debut'] ?? '' ?>&date_fin=<?= $_GET['date_fin'] ?? '' ?>" class="btn">📊 Export CSV</a>
        ...
        <div class="filters no-print">
            ...
            <form method="get" action="index.php">
                ...
                <a href="index.php?controller=patient&action=history&id=<?= $patient['id'] ?>" class="btn btn-danger">Réinitialiser</a>
            </form>
        </div>
        ...
        <div class="result-info">
            ...
            <?php if (count($slips) == 0): ?>
                - <strong>Aucun bulletin trouvé</strong> pour cette période
            <?php else: ?>
                - <strong><?= count($slips) ?> bulletin(s) trouvé(s)</strong>
            <?php endif; ?>
        </div>
        <?php endif; ?>
        ...
        <?php if (empty($slips)): ?>
            <div style="text-align: center; padding: 40px; color: #666;">
                ...
                <?php if (!empty($_GET['date_debut']) || !empty($_GET['date_fin'])): ?>
                    <a href="index.php?controller=patient&action=history&id=<?= $patient['id'] ?>" class="btn btn-blue">Voir tous les bulletins</a>
                <?php endif; ?>
            </div>
        <?php else: ?>
            <?php 
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php if ($line['fichier_path']): ?>
                                                ...
                                                $file_url = 'index.php?controller=slip&action=viewFile&file=' . urlencode($line['fichier_path']);
                                                $icon_class = ($file_ext === 'pdf') ? 'pdf-icon' : 'jpg-icon';
                                                $icon = ($file_ext === 'pdf') ? '📄' : '🖼️';
                                                ?>
                                                <span class="attachment">
                                                    <span class="file-icon <?= $icon_class ?>"><?= $icon ?></span>
                                                    <a href="<?= $file_url ?>" target="_blank"><?= htmlspecialchars($line['fichier_path']) ?></a>
                                                </span>
                                            <?php else: ?>
